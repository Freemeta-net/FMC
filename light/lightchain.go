...
//
// Note if we are running the taerim, fetches the last epoch snapshot header
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *params.TrustedCheckpoint) bool {
	...
	if taerim := lc.hc.Config().Taerim; taerim != nil {
		latest -= latest % taerim.Epoch // epoch snapshot for taerim
	}
	if head >= latest {
		return true
	}
